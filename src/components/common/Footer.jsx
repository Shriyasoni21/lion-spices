        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-[1.4fr_0.8fr_0.8fr_1fr]">
          <div className="space-y-5 text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 md:justify-start">
              {[{ icon: FiInstagram, href: 'https://instagram.com' }, { icon: FiFacebook, href: 'https://facebook.com' }, { icon: FiTwitter, href: 'https://twitter.com' }].map(({ icon: Icon, href }) => (
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-gray-400">
              {productLinks.map((link) => (
                <li key={link.label}><Link to={link.href} className="transition hover:text-white">{link.label}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="mt-5 space-y-3 text-sm text-gray-400">
              <div className="flex items-start gap-3"><FiMapPin className="mt-1 h-4 w-4 text-primary-red" /><div>14-4-274, Joshiwadi, Begum Bazaar, Hyderabad, Telangana, India</div></div>
              <div className="flex items-center gap-3"><FiPhone className="h-4 w-4 text-primary-red" /><a href="tel:+919010782782" className="transition hover:text-white">+91 90107 82782</a></div>
              <div className="flex items-center gap-3"><FiMail className="h-4 w-4 text-primary-red" /><a href="mailto:krishloya789@gmail.com" className="transition hover:text-white">krishloya789@gmail.com</a></div>
            </div>
          </div>
        </div>

        <div className="mt-10 border-t border-white/10 pt-6 text-center text-sm text-gray-500">© {currentYear} Lion Spices. All rights reserved.</div>
      </div>
    </footer>
  );
}
